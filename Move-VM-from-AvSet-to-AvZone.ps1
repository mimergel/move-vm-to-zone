...
.DESCRIPTION
    ...
    The script will snapshot each disk, create a new disk from the snapshot, 
    and create the new VM with the new disks attached.  

    There is no need to reinstall the operating system.

    IMPORTANT: the script does not handle VM extensions or any identities assigned to the Virtual Machine.
...
# change this line in case the name should change with the move
$newvmname = $VirtualMachineName 
# $newvmname = $VirtualMachineName + "-z" + $destzone
...
if ($VerbosePreference -eq "Continue") {
    ...
    {
        $newVM = New-AzVMConfig -VMName $newvmname -VMSize $originalVM.HardwareProfile.VmSize -zone $destzone -Tags $tags -ProximityPlacementGroupId $proximityPlacementGroupID -Verbose
    }
    else {
        $newVM = New-AzVMConfig -VMName $newvmname -VMSize $originalVM.HardwareProfile.VmSize -zone $destzone -Tags $tags -Verbose
    }
    
} else {
    if ($proximityPlacementGroupID -ne "")
    {
        $newVM = New-AzVMConfig -VMName $newvmname -VMSize $originalVM.HardwareProfile.VmSize -zone $destzone -Tags $tags -ProximityPlacementGroupId $proximityPlacementGroupID 
    }
    else {
        $newVM = New-AzVMConfig -VMName $newvmname -VMSize $originalVM.HardwareProfile.VmSize -zone $destzone -Tags $tags 
    }
}
...
write-host "In the next step the VM will be stopped"
write-host "Only proceed when no errors occured before."
...
write-host "In the next step the old VM will be removed and the new VM will be created in the target zone."
write-host "Only proceed when no errors occured before."
